...
namespace LoadAPI
{
    ...
    public class LoadBalancing : Root
    {
        ...
        const string LOGFILENAME = "log-productionplan.txt";
        ...
        public static int LoggerMessage(string m_logMessage)
        {
            try
            {
                StreamWriter sw = new StreamWriter(LOGFILENAME);
                DateTime today = DateTime.Now;
                sw.WriteLine(today+" - "+m_logMessage);
                sw.Close();
            }
            catch (Exception e)
            {
        ...
        public double loadDistribution()
        {
            ...
            {
                LoggerMessage("load demand can't be mate!");
        ...
        public int add(PowerplantExtended powerplant)
        {
            ...
            if (m_payload.load - loadMet() < powerplant.pmin) //adjusting backwards
            {
                ...
                if (currentIndex - 1 < 0)
                {
                    LoggerMessage("No unit commitment coombination can be determined for the load/payload combination");
        ...
        public bool validateAllTurbineLoad()  // validates if all Pmin/Pmax rules are respected
        {
            foreach (PowerplantExtended powerplantExtended in allTurbinesExtended)
            {
                ...
                {
                    LoggerMessage("Not all values of Pmax and Pmin are Respected: "+ powerplantExtended.name);
                    return false;
                }
            }
            return true;
